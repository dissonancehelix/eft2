...
function GM:KeyPress(pl, key)
	if pl:GetObserverMode() ~= OBS_MODE_NONE then
		-- Spectator input: JUMP toggles between chase-cam and free-roam;
		-- auto-follows the ball when switching into chase mode.
		if key == IN_JUMP then
			...
			if mode == OBS_MODE_CHASE then
				...
				for _, ent in ipairs(ents.FindByClass("prop_ball")) do
					if IsValid(ent) then pl:SpectateEntity(ent) end
					break
				end
			end
		end
		-- Block all other spectator keys (Attack1/2 cycle players, etc.)
		return
	end
	if not pl:Alive() then return end
	...
	elseif key == IN_WALK then
		if SERVER and pl:IsIdle() and not pl:IsCarrying() then
			if pl:OnGround() then
				...
				-- Also kick the ragdoll directly so it flies on creation
				-- (SetNextMoveVelocity only moves the player entity on the next tick;
				-- without this the ragdoll spawns with 0 velocity and just drags behind)
				local rag = pl:GetRagdollEntity()
				if IsValid(rag) then
					local rphys = rag:GetPhysicsObject()
					if IsValid(rphys) then rphys:SetVelocityInstantaneous(dir) end
				end
			else
				pl:KnockDown()
			end
